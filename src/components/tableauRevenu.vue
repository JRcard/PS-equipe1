<template>
	<div class="border-2 border-input-border m-5 p-5 rounded-xl bg-card">
		<div class="flex justify-between mb-4">
			...
			<button @click="ajout()" class="w-55 px-3.5 py-2.5 rounded-md font-semibold text-white flex items-center justify-center transition-all duration-300 bg-principale hover:opacity-80 hover:shadow-[0_0_15px_#9034b066]">
	...
	<AjoutRevenu v-if="isVisible" :visible="ajout" />
</template>

<script setup>
import { computed, ref } from "vue";
import AjoutRevenu from "./ajoutRevenu.vue";
