	...
	<div class="max-w-475 mx-auto mt-10">
		...
		<div>
			<div class="flex justify-around md:justify-between my-10 mx-5">
				...
				<button @click="nouveauMois()"
					class="z-99 px-5.5 md:px-8.5 py-2.5 rounded-md font-semibold text-white flex items-center justify-center transition-all duration-300 bg-linear-to-r from-principale to-secondaire hover:shadow-[0_0_15px_#9034b080,0_0_15px_#096cfd80]">NOUVEAU
					MOIS</button>
			</div>

			<div class="flex flex-col gap-3 justify-center">
				<TableauRevenu @totalRevenu="totalRevenue = $event" @deleteID="deleteTransaction"
					@newTransaction="ajoutTransaction" @updateTransaction="updateTransaction" @editThisRow="editThisRow"
					@stopEditing="stopEditing" :data="revenueList" :editingRow="currentEditing" class="z-99" />
				<!-- Des burst de couleur -->
				...
				<TableauDepense @totalExpense="totalExpense = $event" @deleteID="deleteTransaction"
					@newTransaction="ajoutTransaction" @updateTransaction="updateTransaction" @editThisRow="editThisRow"
					@stopEditing="stopEditing" :data="expenseList" :editingRow="currentEditing" class="z-99" />
			</div>
		</div>
	</div>
</template>
