  ...
  <header class="inset-x-0 top-0 z-50">
    <nav aria-label="Global" class="flex items-center justify-between p-6 lg:px-8">
      ...
      <div v-if="!userConnecte" class="hidden lg:flex lg:gap-x-12">
        <RouterLink to="connexion" class="text-base/7 font-semibold text-white z-99 hover:text-secondaire"> Se connecter</RouterLink>
        <RouterLink to="inscription" class="text-base/7 font-semibold text-white z-99 hover:cursor-pointer hover:text-secondaire"> S'inscrire</RouterLink>
      </div>
      <!-- Si l'utilisateur est connecté, on affichce ceci -->
      <div v-else class="hidden lg:flex lg:gap-x-12 items-center">
        <p>Bienvenue ,{{ user.firstName }} {{ user.lastName }}</p>
        <button class="rounded-md px-3.5 py-2.5 text-sm font-semibold text-white shadow-xs hover:bg-secondaire focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-secondaire" @click="deconnexion">SE DECONNECTER</button>
      </div>
    </nav>
    <!--Menu mobile -->
    <el-dialog>
              ...
              <div class="-my-6 divide-y divide-white/10">
                <div class="">
                  ...
                  <div v-if="!userConnecte" class="space-y-2 py-6">
                    <a href="connexion" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-white hover:bg-white/5">Se connecter</a>
                    <a href="inscription" class="-mx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-white hover:bg-white/5">S'inscrire</a>
                  </div>
                  <!-- Si l'utilisateur est connecté, on affichce ceci -->
                  <div v-else class="text-white space-y-2 py-6">
                    <p class="px-3">Bienvenue ,{{ user.firstName }} {{ user.lastName }}</p>
                    <RouterLink class="rmx-3 block rounded-lg px-3 py-2 text-base/7 font-semibold text-white hover:bg-white/5" @click="deconnexion">SE DECONNECTER</RouterLink>
                  </div>
                </div>
              </div>
            </div>
